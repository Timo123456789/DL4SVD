...
def main():
    """
    Main routine for comparing ABB, OBB, and ABB in OBB bounding box formats using mAP@50-95 metrics.

    - Defines the sets and configuration flags.
    - Loads validation and test paths for each model.
    - Extracts mAP@50-95 values for each fold.
    - Identifies the best validation fold per model.
    - Generates boxplots for validation and test results, highlighting best folds.
    - Prints extracted values for inspection.
    """
